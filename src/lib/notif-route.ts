 ...
 * - mémorise d'abord la cible (localStorage) pour que la garde d'accueil
 *   l'honore même si une navigation est perdue pendant un rechargement ;
...
export function openNotifRoute(route?: string): void {
  ...
  try {
    try {
      localStorage.setItem(NOTIF_ROUTE_KEY, target);
    } catch {
...
/**
 * Consomme (lit puis efface) la route de notification mémorisée. Appelée par
 * la garde d'accueil au tout début du chargement, pour aller directement au
 * bon contenu au lieu du dévotionnel par défaut. Renvoie null si aucune.
 */
export function consumeStashedNotifRoute(): string | null {
  try {
    const v = localStorage.getItem(NOTIF_ROUTE_KEY);
    if (v) localStorage.removeItem(NOTIF_ROUTE_KEY);
