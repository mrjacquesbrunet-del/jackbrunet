  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: "linear-gradient(180deg, transparent, rgb(var(--n-900)) 55%)" }}
      >
...
export function GroupsDirectory() {
  ...
  return (
    ...
    <div className="min-h-screen bg-night-800">
      {/* En-tête sombre */}
      <div className="text-cream" style={{ background: `linear-gradient(160deg, ${HEADER_BG}, rgb(var(--n-800)))` }}>
                ...
                <div className="flex flex-wrap gap-2">
                  {(Object.keys(ACCENTS) as AccentKey[]).map((k) => (
                    <button key={k} type="button" onClick={() => setForm((f) => ({ ...f, accent: k }))} aria-label={ACCENTS[k].label}
                      className={`h-8 w-8 rounded-full ring-2 ring-offset-2 ring-offset-night-950 ${form.accent === k ? "ring-cream" : "ring-transparent"}`}
