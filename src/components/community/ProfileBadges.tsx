...
export function ProfileBadgesRow({
  ...
  useEffect(() => {
    let alive = true;
    fetchProfileBadges(userId, streakDays).then((d) => {
      if (alive) setData(d);
    });
    return () => {
      alive = false;
  ...
  if (!data || (earned.length === 0 && !data.weeklyTop)) return null;
...
/** La vitrine : tous les badges, gagnés en métal, verrouillés en grisé avec
 * la progression vers le prochain palier. */
function BadgesVitrine({ data, onClose }: { data: ProfileBadges; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-[110] flex items-end justify-center sm:items-center" role="dialog" aria-modal>
      <style>{BDG_CSS}</style>
      <button type="button" aria-label="Fermer" onClick={onClose} className="absolute inset-0 bg-night-950/70 backdrop-blur-sm" />
      <div className="dark-ctx relative w-full max-w-md rounded-t-3xl border border-white/10 bg-night-900 p-5 pb-[calc(env(safe-area-inset-bottom)+1.25rem)] text-cream sm:rounded-3xl sm:pb-5">
        <div className="flex items-center justify-between">
          <p className="font-display text-lg font-extrabold">Badges</p>
          <button
            type="button"
            onClick={onClose}
            aria-label="Fermer"
        ...
        {data.weeklyTop ? (
          <div className="mt-4 flex items-center gap-3 rounded-2xl border border-dawn-400/40 bg-dawn-400/10 p-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-cream/60">N°1 de la prière sur les 7 derniers jours — remis en jeu chaque semaine.</p>
            </div>
          </div>
        ) : null}
...
function VitrineRow({ s }: { s: BadgeState }) {
  const pct = s.next ? Math.min(100, Math.round((s.count / s.next) * 100)) : 100;
  return (
    <div className="flex items-center gap-3 rounded-2xl bg-white/[0.05] p-3">
      <Medallion kind={s.kind} tier={s.tier} small title={s.label} />
      <div className="min-w-0 flex-1">
        <p className="flex items-baseline justify-between gap-2 text-sm font-bold">
          {s.label}
          <span className={`text-[11px] font-semibold ${s.tier ? "text-dawn-300" : "text-cream/40"}`}>
            {s.tier ? TIER_LABEL[s.tier] : "À débloquer"}
          </span>
        </p>
        <p className="mt-0.5 text-[11px] text-cream/55">
          {s.count.toLocaleString("fr-FR")} {s.detail}
          {s.next ? ` · prochain palier à ${s.next.toLocaleString("fr-FR")}` : " · palier maximum"}
        </p>
