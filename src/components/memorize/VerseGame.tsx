...
const chipCls = `${chunky} border-white/12 bg-night-800 px-4 py-2.5 text-[16px] font-semibold text-cream shadow-[0_4px_0_rgba(255,255,255,0.10)] hover:border-dawn-400/40`;
...
function HappyFace() {
  return (
    <svg viewBox="0 0 48 48" className="h-16 w-16 shrink-0 animate-bounce" aria-hidden="true">
      <circle cx="24" cy="26" r="17" className="fill-dawn-400/20 stroke-dawn-400" strokeWidth="2.5" />
      <path d="M24 2v4M9 7l2.8 2.8M39 7l-2.8 2.8" className="stroke-dawn-300 fill-none" strokeWidth="2.5" strokeLinecap="round" />
      <path d="M15.5 23c1.6-2.2 4.4-2.2 6 0M26.5 23c1.6-2.2 4.4-2.2 6 0" className="stroke-dawn-300 fill-none" strokeWidth="2.5" strokeLinecap="round" />
      <path d="M15.5 30c2.4 4.4 6 6.4 8.5 6.4s6.1-2 8.5-6.4" className="stroke-dawn-300 fill-none" strokeWidth="2.5" strokeLinecap="round" />
    </svg>
  );
}
      ...
      <div className="mt-8 flex items-center gap-3">
        <svg viewBox="0 0 48 48" className="h-12 w-12 -rotate-90">
          ...
          <circle
            ...
            cy="24"
            r={R}
            className="fill-none stroke-dawn-400 transition-all duration-1000 ease-linear"
            strokeWidth="5"
            strokeLinecap="round"
            strokeDasharray={C}
        ...
        <span className="font-game text-lg font-bold tabular-nums text-cream/80">{left}s</span>
      </div>

      <button
        type="button"
        onClick={onReady}
        className={`${chunky} jb-glow mt-8 w-full max-w-xs border-dawn-400 bg-dawn-400 px-5 py-3.5 font-game text-base font-bold uppercase tracking-wide text-night-950 shadow-[0_5px_0_rgba(140,168,0,1)]`}
      >
        C&apos;est mémorisé !
      </button>
      <p className="mt-3 text-xs text-cream/45">Lis-le, retiens-le — il va disparaître.</p>
    </div>
  );
}
...
export function VerseGame({ items, onClose }: { items: MemorizeItem[]; onClose: () => void }) {
  ...
  if (over) {
    const win = lives > 0;
    return (
      <div className="fixed inset-0 z-[80] flex flex-col items-center justify-center overflow-hidden bg-night-950 px-6 text-center font-game text-cream">
        <GameStyles />
        {win ? <Confetti /> : null}
        <div className="jb-pop">
          ...
          <p className="mt-4 text-6xl font-bold text-dawn-400">{score}</p>
          <p className="mt-1 text-sm font-semibold text-cream/55">points gagnés</p>
        </div>
        ...
        <div className="mt-8 w-full max-w-xs space-y-3">
          <button type="button" onClick={restart}
            className={`${chunky} w-full border-dawn-400 bg-dawn-400 px-5 py-3.5 text-base font-bold uppercase tracking-wide text-night-950 shadow-[0_5px_0_rgba(140,168,0,1)]`}>
            Rejouer
          </button>
          <button type="button" onClick={onClose}
            className={`${chunky} w-full border-white/15 bg-night-900 px-5 py-3.5 text-base font-bold uppercase tracking-wide text-cream/80 shadow-[0_5px_0_rgba(255,255,255,0.08)]`}>
            Quitter
          </button>
        </div>
      </div>
  ...
  /* ---------- Leçon plein écran ---------- */
  return (
    <div className="fixed inset-0 z-[80] flex flex-col overflow-hidden bg-night-950 font-game text-cream">
      <GameStyles />
        ...
        <div className="mt-2 flex items-center justify-between text-xs font-bold text-cream/45">
          ...
          <span className="relative flex items-center gap-2">
            ...
            <span className={`inline-flex items-center gap-0.5 ${combo > 1 ? "text-dawn-300" : "text-cream/40"}`}>
              {combo > 1 ? (
                <svg viewBox="0 0 24 24" className="h-3.5 w-3.5 fill-dawn-400" aria-hidden><path d="M12 2c1 3-1 5-2 7s0 5 2 5 3-2 3-4c2 1 3 3 3 5a6 6 0 1 1-11-3c1 2 2 2 2 0 0-3 1-5 3-6z" /></svg>
              ) : null}
              ×{combo}
            </span>
            {/* « +points » qui s'envolent */}
            {floats.map((f) => (
              <span key={f.id} className="pointer-events-none absolute -top-1 right-0 font-bold text-dawn-300"
        ...
        <div className="border-t-2 border-dawn-400/30 bg-dawn-400/[0.12] px-5 pb-[calc(env(safe-area-inset-bottom)+1rem)] pt-4">
          <div className="mx-auto flex w-full max-w-lg items-start gap-4">
            ...
            <div className="min-w-0 flex-1">
              <p className="flex items-center gap-2 font-game text-2xl font-bold text-dawn-300">
              ...
              <button type="button" onClick={continueGame}
                className={`${chunky} jb-glow mt-3 w-full border-dawn-400 bg-dawn-400 px-5 py-3.5 text-center font-game text-base font-bold uppercase tracking-wide text-night-950 shadow-[0_5px_0_rgba(140,168,0,1)]`}>
                Continuer
              </button>
            </div>
          </div>
        ...
        <div className="border-t-2 border-rose-400/40 bg-rose-400/[0.12] px-5 pb-[calc(env(safe-area-inset-bottom)+1.2rem)] pt-4">
          <div className="mx-auto flex w-full max-w-lg items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-cream/70">Une vie en moins — le combo repart à ×1.</p>
            </div>
          </div>
        </div>
      ) : phase === "play" ? (
        <div className="border-t-2 border-white/8 px-5 pb-[calc(env(safe-area-inset-bottom)+1rem)] pt-4">
          <div className="mx-auto w-full max-w-lg">
            <button type="button" disabled
              className="w-full rounded-2xl border-2 border-white/10 bg-white/[0.05] px-5 py-3.5 text-center font-game text-base font-bold uppercase tracking-wide text-cream/30">
              Continuer
            </button>
          </div>
        </div>
